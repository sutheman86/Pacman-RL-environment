...
class PacmanGymEnv(gym.Env):
    ...
    def __init__(self, speedup=1.0, render_mode='rgb_array'):
        ...
        self.observation_space = spaces.Box(low=0, high=255, 
                                            shape=(1, NROWS, NCOLS), dtype=np.uint8)
        ...
        self.grid_state_obj = np.zeros_like((NROWS, NCOLS))
        self.grid_state_pos = np.zeros_like((NROWS, NCOLS))
    ...
    def step(self, action):
        ...
        # 獲取觀察值、回報和完成狀態
        observation = self._get_observation()
        terminated = (self.game.lives == 0)  # Pac-Man 被吃掉時遊戲結束
        truncated = False
    ...
    def _get_observation(self):
        """提取當前畫面並返回觀察數據"""
        # 從遊戲畫面中提取圖像資料
        self.screen = pygame.surfarray.array3d(self.game.screen)
        self.screen = np.transpose(self.screen, (1, 0, 2))  # 調整為 Gym 格式
        obj, pos = self.game.getGameGridState()
        observation = np.expand_dims(obj, axis=0)
        return observation
