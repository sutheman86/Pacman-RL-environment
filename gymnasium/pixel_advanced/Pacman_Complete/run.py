...
class GameController(object):
    def __init__(self, speedup=1.0, headless=False):
        ...
        self.goingbackandforth = False
        self.pacmanstuck = False
        ### REWARD_END
    ...
    def update(self):
        ...
        self.goingbackandforth = self.checkWalkingBackAndForth()

        # REWARD_ADVANCED_3
        if not self.closeToGhost and self.reward <= 0:
            if self.pacman.facing == STOP:
                self.reward -= 8
            if self.goingbackandforth:
                self.reward -= 10
        # REWARD_END
    ...
    # REWARD_ADVANCED_3
    def checkWalkingBackAndForth(self):
    ...
    def checkPelletEvents(self, dt):
        ...
        if pellet:
            ...
            if not self.closeToGhost:
                ...
                if dist < 4 and not (self.goingbackandforth or self.pacmanstuck):
                    self.reward += 2
            # REWARD_END
